// ══════════════════════════════════════════════════════════════
//  Otrebla's Tailoring — Shop Configuration
// ══════════════════════════════════════════════════════════════

const shopConfig = {
  slug: 'otreblas-tailoring',
  name: 'Otrebla\'s Tailoring',
  tagline: 'Expert tailoring for military and formal wear. Precision alterations near Fort Liberty.',
  address: '2720 Bragg Blvd, Fayetteville, NC 28303',
  phone: '(910) 555-0103',
  email: 'info@otreblas.com',
  owner: 'Alberto V.',
  themeColors: {
    primary: '#8e44ad',
    secondary: '#2c3e50',
    accent: '#e67e22'
  },
  story: 'Alberto brings old-world tailoring craftsmanship to the Fort Liberty community. From dress uniforms to formal wear, every garment receives meticulous attention.',
  trustSignals: {
    orders: '500+',
    rating: '4.9'
  },
  enabledServiceIds: null,
  adminPassword: 'puvetr'
};

const employees = [
  { id: 'emp-1', name: 'Alberto V.', role: 'Owner', color: '#8e44ad',
    schedule: { 0:null, 1:{start:'09:00',end:'18:00'}, 2:{start:'09:00',end:'18:00'}, 3:{start:'09:00',end:'18:00'}, 4:{start:'09:00',end:'18:00'}, 5:{start:'09:00',end:'18:00'}, 6:{start:'10:00',end:'15:00'} } },
  { id: 'emp-2', name: 'Carmen D.', role: 'Tailor', color: '#e91e63',
    schedule: { 0:null, 1:{start:'09:00',end:'18:00'}, 2:{start:'09:00',end:'18:00'}, 3:{start:'09:00',end:'14:00'}, 4:{start:'09:00',end:'18:00'}, 5:{start:'09:00',end:'18:00'}, 6:null } },
  { id: 'emp-3', name: 'Luis R.', role: 'Formal Wear', color: '#3f51b5',
    schedule: { 0:null, 1:{start:'10:00',end:'18:00'}, 2:{start:'10:00',end:'18:00'}, 3:{start:'10:00',end:'18:00'}, 4:{start:'10:00',end:'18:00'}, 5:{start:'09:00',end:'16:00'}, 6:{start:'10:00',end:'15:00'} } }
];

const shopHours = {
  0: null,
  1: { start: '09:00', end: '18:00' },
  2: { start: '09:00', end: '18:00' },
  3: { start: '09:00', end: '18:00' },
  4: { start: '09:00', end: '18:00' },
  5: { start: '09:00', end: '18:00' },
  6: { start: '10:00', end: '15:00' }
};

const closedDates = [];
const sharedOrders = [];

function parseTime(str) {
  const [h, m] = str.split(':').map(Number);
  return h * 60 + m;
}

function minutesToTime(mins) {
  const h = Math.floor(mins / 60);
  const m = mins % 60;
  return String(h).padStart(2, '0') + ':' + String(m).padStart(2, '0');
}

function formatTime(str) {
  const [h, m] = str.split(':').map(Number);
  const ampm = h >= 12 ? 'PM' : 'AM';
  const hr = h === 0 ? 12 : h > 12 ? h - 12 : h;
  return hr + ':' + String(m).padStart(2, '0') + ' ' + ampm;
}

function parseDuration(durStr) {
  const match = durStr.match(/(\d+)/);
  return match ? parseInt(match[1], 10) : 30;
}

function isDayClosed(dateStr) {
  if (closedDates.includes(dateStr)) return true;
  const d = new Date(dateStr + 'T00:00:00');
  return !shopHours[d.getDay()];
}
